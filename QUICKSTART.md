...
【BTCUSDT】
净收益(4h): 0.45% (阈值: 0.40%)
高费率: Binance 0.10% (8.0h) [4h: 0.05%]
低费率: OKX -0.05% (8.0h) [4h: -0.025%]
价差比: -0.30%
...
1. **获取周期**：自动获取每个合约的资金费率结算周期（1h、4h、8h等）
2. **统一转换**：将所有费率转换为4小时费率
   - 公式：4h费率 = 原始费率 × (4 / 结算周期小时数)
   - 示例：1h周期的-1%费率 → 4h费率 = -1% × 4 = -4%
3. **找差异**：同一币种在不同交易所的4小时费率差异
4. **算收益**：净收益 = 4h费率差 - 价差
5. **抓机会**：净收益 > 0.4% 时通知
...
- **智能周期识别**：自动获取每个合约的真实结算周期
- **统一费率计算**：所有费率转换为4小时费率进行比较
- **实时监控**：每10秒更新数据
- **定期更新**：每小时更新结算周期信息
